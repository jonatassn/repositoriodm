...
class TaskLocalDataSource : TaskDataSource {
    ...
    override fun getAll(): Single<List<Task>> {
        return taskDao.getAll().subscribeOn(Schedulers.io())
    }


    override fun insert(task: Task): Single<Long> {
        return taskDao.insert(task).subscribeOn(Schedulers.io())
    }

    override fun update(task: Task): Completable {
        return taskDao.update(task).subscribeOn(Schedulers.io())
    }

    override fun remove(task: Task): Completable {
        return taskDao.remove(task).subscribeOn(Schedulers.io())
    }
